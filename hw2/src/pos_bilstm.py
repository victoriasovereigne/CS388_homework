...
class Model:
	def __init__(self, input_dim, sequence_len, output_dim, 
				prefix_dim, suffix_dim, hidden_state_size=300):
	...
	# ============================
	# Get the mask for OOV words
	# ============================
	def get_oov_mask(self, t, oov_code):
		mask = tf.cast(tf.equal(t, oov_code), tf.int32)
		lengths = tf.reduce_sum(mask, reduction_indices=1)
		return mask, lengths
	...
	# Adapted from https://github.com/monikkinom/ner-lstm/blob/master/model.py __init__ function
	# =======================================================
	# Modified: adding 2 parameters - features and concat_type
	# features is a dictionary with keys: 'prefix', 'suffix', and 'others'
	# concat_type is a string: either 'input' or 'output' to determine where to concat the features
	# =======================================================
	def create_graph(self, features, concat_type):
		...
		# ============================
		# Get the OOV mask and OOV total length
		# ============================
		...
		# Convert prefixes into one hot vectors
		# ============================
		# Checks if we use 'prefix' as orthographic features
		# ============================
		if 'prefix' in features.keys():
			with tf.variable_scope("lstm_prefix"):
				self._prefix_words = tf.placeholder(tf.int32, [BATCH_SIZE, self._sequence_len])
				lstm_prefix = self.one_hot(self._prefix_words, self._prefix_dim)
				if concat_type == 'input': # only concat with lstm_input if the user wants to 
					lstm_input = tf.concat([lstm_input, lstm_prefix], 2)

		# Convert suffixes into one hot vectors
		# ============================
		# Checks if we use 'suffix' as orthographic features
		# ============================
		if 'suffix' in features.keys():
			with tf.variable_scope("lstm_suffix"):
				self._suffix_words = tf.placeholder(tf.int32, [BATCH_SIZE, self._sequence_len])
				lstm_suffix = self.one_hot(self._suffix_words, self._suffix_dim)
				if concat_type == 'input': # only concat with lstm_input if the user wants to 
					lstm_input = tf.concat([lstm_input, lstm_suffix], 2)

		# ============================
		# Checks if we use 'others' as orthographic features
		# ============================
		if 'others' in features.keys():
			with tf.variable_scope("lstm_others"):
				self._other_words = tf.placeholder(tf.int32, [BATCH_SIZE, self._sequence_len, 3])
				lstm_others = tf.cast(self._other_words, tf.float32)
				if concat_type == 'input': # only concat with lstm_input if the user wants to 
					lstm_input = tf.concat([lstm_input, lstm_others], 2)
		...
		with tf.variable_scope("lstm_output"):
			...
			# only concat with lstm_output if the user wants to 
			if concat_type == 'output':
				if 'prefix' in features.keys():
					outputs = tf.concat([outputs, lstm_prefix], 2)
				if 'suffix' in features.keys():
					outputs = tf.concat([outputs, lstm_suffix], 2)
				if 'others' in features.keys():
					outputs = tf.concat([outputs, lstm_others], 2)
		...
		# Adding the OOV accuracy and loss
		...
		self._oov_average_loss = self._loss/tf.cast(self._oov_total_length, tf.float32)
	...
	# Convert a prefix/suffix encoding into a one-hot vector
	def one_hot(self, suffixes, dim):
		suffixes = tf.cast(suffixes, tf.int32)
		s_one_hot = tf.one_hot(suffixes, dim)
		s_one_hot = tf.cast(s_one_hot, tf.float32)
...
def generate_batch(X, y, features):
	for i in xrange(0, len(X), BATCH_SIZE):
		...
		if 'prefix' in features.keys():
			ff['prefix'] = features['prefix'][i:i+BATCH_SIZE]

		if 'suffix' in features.keys():
			ff['suffix'] = features['suffix'][i:i+BATCH_SIZE]

		if 'others' in features.keys():
			ff['others'] = features['others'][i:i+BATCH_SIZE]
...
def shuffle_data(X, y, features):
	...
	XX = [X[num] for num in ran]
	yy = [y[num] for num in ran]
	ff = {}

	if 'prefix' in features.keys():
		ff['prefix'] = [features['prefix'][num] for num in ran]

	if 'suffix' in features.keys():
		ff['suffix'] = [features['suffix'][num] for num in ran]

	if 'others' in features.keys():
		ff['others'] = [features['others'][num] for num in ran]

	return XX, yy, ff

def generate_epochs(X, y, features, no_of_epochs):
	lx = len(X)
	...
	if 'prefix' in features.keys():
		features['prefix'] = features['prefix'][:lx]

	if 'suffix' in features.keys():
		features['suffix'] = features['suffix'][:lx]

	if 'others' in features.keys():
		features['others'] = features['others'][:lx]

	for i in range(no_of_epochs):
		shuffle_data(X, y, features)
		yield generate_batch(X, y, features)

## Compute overall loss and accuracy on dev/test data
def compute_summary_metrics(sess, m, sentence_words_val, sentence_tags_val, features_val):
	loss, accuracy, total_len = 0.0, 0.0, 0
	oov_accuracy = 0.0
	total_oov_len = 0
	for i, epoch in enumerate(generate_epochs(sentence_words_val, sentence_tags_val, features_val, 1)):
		for step, (X, y, features) in enumerate(epoch):
			mydict = {m.input_words:X, m.output_tags:y}

			if 'prefix' in features.keys():
				mydict[m.prefix_words] = features['prefix']

			if 'suffix' in features.keys():
				mydict[m.suffix_words] = features['suffix']

			if 'others' in features.keys():
				mydict[m.other_words] = features['others']

			batch_loss, batch_accuracy, batch_oov_accuracy, batch_len, batch_oov_len = \
			sess.run([m.loss, m.accuracy, m.oov_accuracy, m.total_length, m.oov_total_length], \
					feed_dict=mydict)
			loss += batch_loss
...
def train(sentence_words_train, sentence_tags_train, features_train,
			sentence_words_val, sentence_tags_val, features_val,
			vocab_size, no_pos_classes, no_prefix, no_suffix,
			train_dir, concat_type):
	m = Model(vocab_size, MAX_LENGTH, no_pos_classes, no_prefix, no_suffix)
	with tf.Graph().as_default():
	    ...
	    m.create_graph(features_train, concat_type)
	    ## create training op
	    ...
	    j = 0
	    for i, epoch in enumerate(generate_epochs(sentence_words_train, sentence_tags_train, features_train, NO_OF_EPOCHS)):
	        ...
	        for step, (X, y, features) in enumerate(epoch):
	        	...
	        	# ===========================================
	        	# Check which orthographic features are used
	        	# before feeding to feed_dict
	        	# ===========================================
	        	if 'prefix' in features.keys():
	        		mydict[m.prefix_words] = features['prefix']
	        	if 'suffix' in features.keys():
				...
				_, summary_value = sess.run([train_op, summary_op], feed_dict=mydict)
				duration = time.time() - start_time
				j += 1
				if j % VALIDATION_FREQUENCY == 0:
					val_loss, val_accuracy, val_oov_accuracy = compute_summary_metrics(sess, m, sentence_words_val, sentence_tags_val, features_val)
					summary = tf.Summary()
					...
					summary.value.add(tag='OOV Validation Accuracy', simple_value=val_oov_accuracy)
					summary_writer.add_summary(summary, j)
					log_string = '{} batches ====> Validation Accuracy {:.3f}, Validation Loss {:.3f}, Validation OOV Accuracy {:.3f}, Time {:.3f}'
					print log_string.format(j, val_accuracy, val_loss, val_oov_accuracy, time.time())
				else:
...
def test(sentence_words_test, sentence_tags_test, features_test,
		 vocab_size, no_pos_classes, no_prefix, no_suffix,
		 train_dir, concat_type):
	m = Model(vocab_size, MAX_LENGTH, no_pos_classes, no_prefix, no_suffix)
	with tf.Graph().as_default():
		global_step = tf.Variable(0, trainable=False)
		m.create_placeholders()
		m.create_graph(features_test, concat_type)
		...
		with tf.Session() as sess:
			...
			test_loss, test_accuracy, test_oov_accuracy = compute_summary_metrics(sess, m, sentence_words_test,
															   sentence_tags_test, features_test)
...
if __name__ == '__main__':
	...
	features_type = sys.argv[5]
	concat_type = sys.argv[6]
	...
	X_train, y_train, p_train, s_train, o_train, _ = p.get_processed_data(train_mat, MAX_LENGTH)
	X_val, y_val, p_val, s_val, o_val, _ = p.get_processed_data(val_mat, MAX_LENGTH)
	X_test, y_test, p_test, s_test, o_test, _ = p.get_processed_data(test_mat, MAX_LENGTH)

	features_train = {}
	if 'p' in features_type:
		features_train['prefix'] = p_train

	if 's' in features_type:
		features_train['suffix'] = s_train

	if 'o' in features_type:
		features_train['others'] = o_train

	features_val = {}
	if 'p' in features_type:
		features_val['prefix'] = p_val

	if 's' in features_type:
		features_val['suffix'] = s_val

	if 'o' in features_type:
		features_val['others'] = o_val

	features_test = {}
	
	if 'p' in features_type:
		features_test['prefix'] = p_test
	
	if 's' in features_type:
		features_test['suffix'] = s_test

	if 'o' in features_type:
		features_test['others'] = o_test
	...
	# 	os.mkdir(train_dir)
	# 	train(X_train, y_train, features_train, X_val, y_val, features_val, 
	# 			len(p.vocabulary)+2, len(p.pos_tags)+1, 
	# 			len(p.prefixes)+1, len(p.suffixes)+1, train_dir, concat_type)
	# else:
	# 	test(X_test, y_test, features_test, len(p.vocabulary)+2, len(p.pos_tags)+1, 
	# 		len(p.prefixes)+1, len(p.suffixes)+1, train_dir, concat_type)
	# end = time.time()
	# print end-start
